...
//  zetas.imbue( norm_rand );
  ...
  theta = zetas*sqrt(vartheta) + vartheta*(Z*as + oneN*(apb+mu_theta*Sigma_theta_inv));
...
Rcpp::List update_ab_NA(unsigned int N,unsigned int J,const arma::mat& Z,arma::vec& as,arma::vec& bs,
  ...
  as(j) = R::qnorm(pa + ua*(1.0-pa),mu_xi_star(0),sqrt(Sig_xi_star(0,0)),1,0) ;

  ub = R::runif(0.0,1.0);
  mb_a = mu_xi_star(1) + Sig_xi_star(0,1)/Sig_xi_star(1,1)*(as(j) - mu_xi_star(0));
  vb_a = Sig_xi_star(1,1) - Sig_xi_star(0,1)*Sig_xi_star(0,1)/Sig_xi_star(0,0);
...
//          sj = (1-W(i))*Y(i,j) + sj;
          ...
          Z(i,j) = R::qnorm(p4pno(0) + uZ*( p4pno(1)-p4pno(0) ),eta(i),1.0,1,0) ;
//        }
      }
   ...
   //draw g from marginal distribution w Gibbs within Gibbs
      ...
      for(unsigned int r =0;r<gwg_reps;r++){
        pg_temp = R::pbeta(1.0-s_temp,n01+alpha_c,n0dot-n01+beta_c,1,0);
        g_temp  = R::qbeta(R::runif(0,1)*pg_temp,n01+alpha_c,n0dot-n01+beta_c,1,0);
        ps_temp = R::pbeta(1.0-g_temp,n10+alpha_s,n1dot-n10+beta_s,1,0);
...
Rcpp::List Gibbs_4PNO(const arma::mat& Y,const arma::vec& mu_xi,const arma::mat& Sigma_xi_inv,
  ...
  double beta_s,unsigned int burnin,const arma::vec& cTF,const arma::vec& sTF,
  unsigned int gwg_reps,unsigned int chain_length=10000){
...
//need to initialize, theta, as, bs, kappas, Z; eventually gs
  ...
  for(unsigned int t = 0; t < chain_length; t++){
    ...
    //    Rcpp::List ste2ab = update_ab_norestriction(N,J,Z,as,bs,theta,mu_xi,Sigma_xi_inv);//I(alpha>0) removed

    //Update theta. Theta is stored in memory and function should update each iteration too.
    Rcpp::List step3theta = update_theta(N,Z,as,bs,theta,mu_theta,Sigma_theta_inv);
 ...
 return Rcpp::List::create(Rcpp::Named("EAPtheta",EAPtheta),
...
Rcpp::List update_2pno(unsigned int N,unsigned int J,const arma::mat& Y,arma::mat& Z,const arma::vec& as,
  ...
  const arma::mat& Sigma_xi_inv,const double& mu_theta,const double& Sigma_theta_inv){
  ...
  for(unsigned int j=0;j<J;j++){
    ...
    ua = R::runif(0.0,1.0);
    pa = R::pnorm(0.0,mu_xi_star(0),sqrt(Sig_xi_star(0,0)), 1, 0);
    as1(j) = R::qnorm(pa + ua*(1.0-pa),mu_xi_star(0),sqrt(Sig_xi_star(0,0)),1,0) ;
    ub = R::runif(0.0,1.0);
    mb_a = mu_xi_star(1) + Sig_xi_star(0,1)/Sig_xi_star(1,1)*(as(j) - mu_xi_star(0));
    vb_a = Sig_xi_star(1,1) - Sig_xi_star(0,1)*Sig_xi_star(0,1)/Sig_xi_star(0,0);
  ...
  arma::vec zetas(N,arma::fill::randn);
  arma::vec theta1 = zetas*sqrt(vartheta) + vartheta*(Z*as1 + oneN*(apb+mu_theta*Sigma_theta_inv));

  return Rcpp::List::create(Rcpp::Named("as1",as1),
                            Rcpp::Named("bs1",bs1),
...
//need to initialize, theta, as, bs, kappas, Z; eventually gs
  ...
  for(unsigned int t = 0; t < chain_length; t++){
    //Generate Z matrix. Note that Z will be overwritten to disk here.
    Rcpp::List step1Z = update_2pno(N,J,Y,Z,as,bs,theta,KAPS,mu_xi,Sigma_xi_inv,mu_theta,Sigma_theta_inv);
